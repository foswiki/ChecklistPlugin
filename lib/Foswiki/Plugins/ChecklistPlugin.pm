...
$REVISION = '1.026'
  ; #dro# added timestamp feature requested by TWiki:Main.VickiBrown; fixed uninitialized value bugs;
...
### my ( $text, $topic, $web ) = @_;   # do not uncomment, use $_[0], $_[1]... instead
    ...
    $_[0] =~
s/<\/head>/<script src="%PUBURL%\/%SYSTEMWEB%\/$pluginName\/itemstatechange.js" language="javascript" type="text\/javascript"><\/script><\/head>/is
      unless ( $_[0] =~ /itemstatechange.js/ );
    &handleAllTags(@_);
}
...
sub urlEncode {
    my ($txt) = @_;
    $txt =~
      s/([^A-Za-z0-9\$\-\_\.\+\!\*\'\(\)\,])/sprintf("%%%02X", ord($1))/seg
      if defined $txt;
    return $txt;
}
