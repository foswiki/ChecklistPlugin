...
### my ( $text, $topic, $web ) = @_;   # do not uncomment, use $_[0], $_[1]... instead
    ...
    if( $Foswiki::Plugins::VERSION < 2.3 ) {
        $scr = "<script src='%PUBURLPATH/%SYSTEMWEB%/$pluginName/itemstatechange.js'></script>";
    }
    else {
        $scr = "<script src='%PUBURLPATH{topic=\"%SYSTEMWEB%.$pluginName\" \"itemstatechange.js\"}%'></script>";
    }
...
sub initOptions() {
	...
	%options = ( );
		...
		next unless defined $options{$option};
			...
			$options{$option}=&Foswiki::Func::expandCommonVariables($options{$option},$topic, $web);
...
sub initNamedDefaults {
	...
	foreach my $default (keys %globalDefaults) {
		...
		$namedDefaults{$name}{$default}= (&Foswiki::Func::getPreferencesValue("\U${pluginName}_TEMPLATE_${tmplName}_${default}\E") || undef)  unless (!defined $tmplName) || (defined $params{$default});
...
sub handleChecklist {
	...
	Foswiki::Func::writeDebug("- ${pluginName}::handleChecklist($attributes,...refText...)") if $debug;
...
sub handleChecklistItem {
	...
	Foswiki::Func::writeDebug("- ${pluginName}::handleChecklistItem($attributes)") if $debug;
...
sub getNextState {
	...
	Foswiki::Func::writeDebug("- ${pluginName}::getNextState($name, $lastState)=$state; allstates=".$options{states}) if $debug;
...
sub checkChangeAccessPermission {
	...
	if ( ! &Foswiki::Func::checkAccessPermission($perm, $user, $text, $checkTopic, $web)) {
		$ret = 0;

		eval { require Foswiki::AccessControlException; };
		if ($@) {
			Foswiki::Func::redirectCgiQuery(Foswiki::Func::getCgiQuery(),Foswiki::Func::getOopsUrl($web,$checkTopic,"oopsaccesschange"));
		} else {
			require Error;
...
sub doChecklistItemStateReset {
	my ($n, $state, $text) = @_;
	Foswiki::Func::writeDebug("- ${pluginName}::doChecklistItemStateReset($n,$state,...text...)") if $debug;
...
sub doChecklistItemStateChange {
	...
	Foswiki::Func::writeDebug("- ${pluginName}::doChecklistItemStateChange($id,$n,$lastState,...text...)") if $debug;
...
sub renderChecklistItem {
	Foswiki::Func::writeDebug("- ${pluginName}::renderChecklistItem()") if $debug;
	my $query = &Foswiki::Func::getCgiQuery();
	my $text = "";
	my $name = $options{'name'};
...
sub substIllegalChars {
	my ($txt) = @_;
    $txt = join('', grep( $Foswiki::regex{mixedAlphaNum}, split('',$txt))) if defined $txt;
	return $txt;
}
# ========================
sub getImageSrc {
	...
	if ($txt=~/^(?:<span.*?>)?([^<]*)<img[^>]+?src=(["'])([^\2'">]+?)\2[^>]*>(.*)(?:<\/span>)?$/is) {
		##$src=$1;
		($b,$src,$a)=($1,$3,$4);
	}
	return ($src,$b,$a);
...
sub readChecklistItemStateTopic {
	...
	Foswiki::Func::writeDebug("- ${pluginName}::readChecklistItemStateTopic($topic, $web): $clisTopicName") if $debug;

	my $clisTopic = Foswiki::Func::readTopicText($web, $clisTopicName);

	if ($clisTopic =~ /^http.*?\/oops/) {
		Foswiki::Func::redirectCgiQuery(Foswiki::Func::getCgiQuery(), $clisTopic);
		return;
	}
...
sub getLogEntry {
	...
	my $logentry = Foswiki::Func::expandCommonVariables($format, $options{logtopic}, $web);
...
sub saveChecklistItemStateTopic {
	...
	Foswiki::Func::writeDebug("- ${pluginName}::saveChecklistItemStateTopic($name): $clisTopicName, ".$namedDefaults{$name}{'statetopic'}) if $debug;
	...
	foreach my $n ( sort keys %{ $idMapRef } ) {
		...
		$states = &Foswiki::Func::getPreferencesValue("\U$pluginName\E_STATES") unless defined $states && $states ne "";
	...
	Foswiki::Func::saveTopicText($web, $clisTopicName, $topicText, 1, !$options{'notify'});
...
sub collectAllChecklistItems {
	...
	Foswiki::Func::writeDebug( "- ${pluginName}::collectAllChecklistItems()" ) if $debug;
	...
	Foswiki::Func::writeDebug( "- ${pluginName}::collectAllChecklistItems() done!" ) if $debug;
}
# =========================
